...
// 18:00 と 23:00 に実行: Firestore から生成待ち素材を取得 → GCS から画像取得
//                       → Claude 生成 → 動画生成 → Firestore 更新 → Chat 通知

import { Request, Response } from 'express';
import { logger } from '../utils/logger';
import { downloadImagesFromGcs } from './storageHandler';
...
async function getAppUrl(): Promise<string> {
  const serviceUrl = process.env.CLOUD_RUN_URL
    || `https://sns-auto-post-${process.env.GOOGLE_CLOUD_PROJECT}.a.run.app`;
...
async function validateSchedulerRequest(req: Request): Promise<boolean> {
  ...
  // Cloud Scheduler の OIDC Bearer トークンによる認証
...
export async function handleProcess(req: Request, res: Response): Promise<void> {
  ...
  // 非同期で処理（Cloud Scheduler のタイムアウトを避けるため即時 200 を返す）
...
async function processAllMaterials(): Promise<void> {
  // Firestore から生成待ち素材を取得
  const materials = await listPendingMaterials();

  if (materials.length === 0) {
    logger.info('生成待ち素材なし。処理をスキップします');
    return;
  }
...
async function processMaterial(material: PendingMaterialRecord): Promise<void> {
  logger.info('素材を処理中', { materialId: material.materialId });

  try {
    // GCS から画像をダウンロード（最大4枚）
    const images = material.gcsImagePaths.length > 0
      ? await downloadImagesFromGcs(material.gcsImagePaths)
      : [];
    ...
    if (images.length > 0) {
      try {
        videoGcsPath = await generateSlideshowVideo(
          material.materialId,
          images,
          generated.instagram   // Instagram 投稿文を字幕として使用
        );
      } catch (videoErr) {
    ...
    // Firestore に生成結果を保存
    await updateWithGeneratedContent(material.materialId, generated, videoGcsPath);

    logger.info('素材の処理が完了', { materialId: material.materialId });

  } catch (err) {
    // 生成失敗をマークして再処理を防ぐ
    await markGenerationFailed(material.materialId).catch(() => {});
    throw err;
  }
}
